...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto update(Long userId, Long itemId, ItemDto itemDto) {
        ...
        if (!item.getOwner().getId().equals(userId)) throw new NotFoundException("Not an owner");
    ...
    public List<ItemDto> getAllByOwnerId(Long ownerId, Integer from, Integer size) {
        ...
        return itemRepository.findByOwner(ownerId, page).stream().map(item -> {
            ItemDto dto = ItemMapper.toItemDto(item);
    ...
    private void setBookings(ItemDto dto, List<Booking> bookings) {
        ...
        Booking last = bookings.stream().filter(b -> b.getStart().isBefore(now))
                .reduce((first, second) -> second).orElse(null);
        Booking next = bookings.stream().filter(b -> b.getStart().isAfter(now))
